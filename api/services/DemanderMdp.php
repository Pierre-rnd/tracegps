...
{	$msg = "Erreur : méthode HTTP incorrecte.";
    ...
    $nouveauMdp = substr(str_shuffle('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'), 0, 8);

    $mdpCrypte = sha1($nouveauMdp);

    $sql = "UPDATE users SET password = :password WHERE email = :email";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([':password' => $mdpCrypte, ':email' => $email]);

    $sujet = "Votre nouveau mot de passe";
    $message = "Bonjour,\n\nVoici votre nouveau mot de passe : $nouveauMdp\n\nPensez à le changer après connexion.";
    $headers = "From: no-reply@tonsite.fr";
